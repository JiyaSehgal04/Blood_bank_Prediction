...
class XGBForecaster:
    ...
    def fit(self, df: pd.DataFrame, feature_cols: list) -> dict:
        ...
        within_20pct = float(np.mean(np.abs(y - preds) <= np.maximum(y * 0.2, 1)) * 100)
        return {
            "mae": round(mae, 3),
            "rmse": round(rmse, 3),
...
class ModelTrainer:
    ...
    def _store_predictions(
        ...
        if not os.environ.get("SUPABASE_URL") or not os.environ.get("SUPABASE_KEY"):
            print("  Warning: prediction store skipped: Supabase credentials are not configured")
            return 0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a local CSV/XLSX register file for offline training",
    )
    parser.add_argument(
        "--from-file",
        dest="from_csv",
        type=str,
        help="Alias for --from-csv",
    )
