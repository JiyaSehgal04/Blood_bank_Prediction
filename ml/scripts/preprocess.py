...
from shared.cleaning_utils import VALID_BLOOD_GROUPS, clean_raw_rows
...
class FeaturePipeline:
    ...
    def _fetch_query(self, query_factory) -> list:
        rows = []
        start = 0
        while True:
    ...
    def fetch_daily_summary(self) -> pd.DataFrame:
        rows = self._fetch_query(
            lambda: self._get_client().table("daily_summary").select("*").order("summary_date")
        )
        if not rows:
            return pd.DataFrame()
    ...
    def _build_features_from_summary(
        self,
        raw: pd.DataFrame,
        ...
        # ── inventory features (live/latest, same value for all rows) ─────────
    ...
    def build_features(self, component: str = "WB/PRC") -> pd.DataFrame:
        ...
        return self._build_features_from_summary(raw, component)

    def build_features_from_file(self, file_path: str, component: str = "WB/PRC") -> pd.DataFrame:
        """
        Build features from a local raw register file (.csv, .xlsx, .xls).
        The register contains collected units, so local training uses daily unit
        counts as the demand signal when Supabase daily_summary is unavailable.
        """
        path = Path(file_path)
        ...
        valid, _, _ = clean_raw_rows(headers, raw_rows, source="bulk_load")
        if not valid:
            return pd.DataFrame()

        records = pd.DataFrame(valid)
        records["collection_date"] = pd.to_datetime(records["collection_date"])
        records["expiry_date"] = pd.to_datetime(records["expiry_date"], errors="coerce")
        records["units_received"] = 1
        records["units_issued"] = 1
        records["units_expired"] = 0
        records["unmet_requests"] = 0

        summary = (
            records.groupby(["collection_date", "component", "blood_group"], as_index=False)
            .agg(units_received=("units_received", "sum"), units_issued=("units_issued", "sum"))
            .rename(columns={"collection_date": "summary_date"})
        )
        summary["units_expired"] = 0
        summary["unmet_requests"] = 0
        summary["closing_stock"] = (
            summary.sort_values("summary_date")
            .groupby(["component", "blood_group"])["units_received"]
            .cumsum()
        )

        today = pd.Timestamp(date.today())
        available = records[records["expiry_date"].isna() | (records["expiry_date"] >= today)]
        expiring = available[
    ...
    def build_features_from_csv(self, csv_path: str, component: str = "WB/PRC") -> pd.DataFrame:
        return self.build_features_from_file(csv_path, component)
