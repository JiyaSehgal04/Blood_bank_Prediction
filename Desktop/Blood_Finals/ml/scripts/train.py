...
Model routing (cold-start strategy):
  ...
  Days   30+ : Random Forest (RF) replaces SES as primary forecaster
               Ensemble = 0.7 × RF + 0.3 × SES
...
class SESForecaster:
    ...
    def __init__(self, alpha: float = SES_ALPHA):
        self.alpha  = alpha
        self.state  = {bg: INDIAN_BLOOD_DIST.get(bg, 0.01) * 20
                       for bg in VALID_BLOOD_GROUPS}   # baseline ~20 units/day scaled
...
class RFForecaster:
    ...
    @staticmethod
    def load(path: Path) -> "RFForecaster":
        return joblib.load(path)
...
class AnomalyDetector:
    ...
    def __init__(self):
        ...
        self.trained = False
        self.feature_cols = ["day_of_week", "units_demanded",
...
class ModelTrainer:
    ...
    def train_component(self, component: str) -> dict:
        ...
        for bg in VALID_BLOOD_GROUPS:
            ...
            ses_models[bg] = ses
        joblib.dump(ses_models, MODELS_DIR / f"ses_{component.replace('/', '_')}.joblib")
        print(f"  SES trained for all blood groups.")
        result["ses"] = "trained"

        # ── Isolation Forest (14+ days) ───────────────────────────────────────
        if n_days >= 14:
            anomaly = AnomalyDetector()
            # rename for anomaly detector
            df_a = df.rename(columns={"units_demanded": "units_demanded",
                                       "rolling_avg_7d": "rolling_avg_7d",
                                       "rolling_std_7d": "rolling_std_7d"})
            anomaly.fit(df_a)
            anomaly.save(MODELS_DIR / f"isolation_forest_{component.replace('/', '_')}.joblib")
            ...
            result["isolation_forest"] = {
                ...
                "anomaly_dates": anomalies["summary_date"].dt.date.astype(str).tolist()[:10],
            }
        else:
            print(f"  Isolation Forest skipped ({n_days} days < 14 required)")
            result["isolation_forest"] = {"status": "skipped", "reason": f"only {n_days} days"}

        # ── Random Forest (30+ days) ──────────────────────────────────────────
        if n_days >= MIN_ROWS_FOR_RF:
            rf = RFForecaster()
            metrics = rf.fit(df, feature_cols)
            rf.save(MODELS_DIR / f"rf_{component.replace('/', '_')}.joblib")
            print(f"  Random Forest trained. MAE={metrics['mae']}, "
                  f"RMSE={metrics['rmse']}, Coverage={metrics['coverage_20pct']}%")
            result["random_forest"] = {"status": "trained", **metrics}
        else:
            print(f"  Random Forest skipped ({n_days} days < {MIN_ROWS_FOR_RF} required)")
        ...
        n_stored = self._store_predictions(df, component, ses_models,
                                            rf if n_days >= MIN_ROWS_FOR_RF else None)
        result["predictions_stored"] = n_stored
        return result

    def _store_predictions(self, df: pd.DataFrame, component: str,
                            ses_models: dict,
                            rf: "RFForecaster | None") -> int:
        feature_cols = self.pipe.get_feature_cols()
        records      = []
        today        = date.today()
        ...
        for bg in VALID_BLOOD_GROUPS:
            ...
            if rf and rf.trained:
                rf_pred  = rf.predict(last_row)
                final    = round(0.7 * rf_pred + 0.3 * ses_pred, 1)
                model_used = "ensemble_rf_ses"
            else:
                final    = ses_pred
                model_used = "ses"
